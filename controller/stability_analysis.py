...
kc1_test_range = range(0,-500,-1)
kc2_test_range = 0

# define initial arrays
def stability_analysis(KP1):
    # define initial arrays
...
for kc in kc1_test_range:
    k,err = stability_analysis(kc)
    if err < min_ier1[1]:
        min_ier1 = (k,err)
    if err < 2:
        stable_kc1.append(k)
    kc1_stab.append((k,err))

kc, ier = zip(*kc1_stab)

print(f"best kc1: {min_ier1}")
print(f"stable kc range: {min(stable_kc1)}, {max(stable_kc1)}")

plt.scatter(kc,ier)
plt.plot(kc, np.ones(len(kc1_test_range))*2, "k--",linewidth="1", label="acceptable error")
plt.xlabel("Kc Value")
